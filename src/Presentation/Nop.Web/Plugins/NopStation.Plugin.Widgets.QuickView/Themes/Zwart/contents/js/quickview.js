...
var QuickView = {
  ...
  render_quickview_button: function (label) {
    $('.product-item[data-quick-view!="true"]').each(function (i, obj) {
      ...
      var html = '<i><a  class="quick-view-plugin-btn btn button-2 text-white" data-productid="' + productid + '" title="' + label + '"><i class="fa fa-eye" aria-hidden="true"></i></a></i>';
      $(this).children('.details').children('.add-info').children('.products-box-new-icon-set').children('.products-box-icon').append(html);
      $(this).attr("data-quick-view", true);
    });

    $('.quick-view-plugin-btn').click(function (event) {
      var productid = $(this).data('productid');
