...
var QuickView = {
  ...
  render_quickview_button: function (label) {
    $('.product-item[data-quick-view!="true"]').each(function (i, obj) {
      ...
      var html = '<button type="button" class="button-2 quick-view-btn" data-productid="' + productid + '" title="' + label + '"><span class="icon icon-quick-view"></span></button>';
      $(this).children('.picture').children('.buttons-hover').children('.add-Cart').prepend(html);
